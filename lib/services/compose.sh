...
append_wikiless() {
  ...
  if [[ "${TEST_MODE:-false}" == "true" ]]; then
    redis_url="redis://${CONTAINER_PREFIX}wikiless_redis:6379"
  fi
...
append_invidious() {
  ...
  if [[ "${TEST_MODE:-false}" == "true" ]]; then
    companion_url="http://${CONTAINER_PREFIX}companion:8282/companion"
  fi
...
append_immich() {
  ...
  if [[ "${TEST_MODE:-false}" == "true" ]]; then
    ml_url="http://${CONTAINER_PREFIX}immich-ml:3003"
  fi
