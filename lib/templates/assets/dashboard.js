      ...
      .map(([id, meta]) => {
        ...
        if (id === 'immich' || id === 'immich-server') {
            normalized.chips.push({
                ...
                variant: 'tertiary', 
                icon: 'shield',
                tooltip: 'Your photos and videos are stored locally. Internet access is only used for maps and downloading smart sorting/tagging models.'
            });
        }
...
function createServiceCard(id, meta, hardened = false) {
  ...
  const containerInfo = containerIds[portainerLookupId] || containerIds[id] || containerIds['hub-' + portainerLookupId];
  ...
  const pChip = createChipElement(id, {
    ...
    icon: 'settings',
    variant: 'admin tonal',
    tooltip: hasContainerId ? 'Direct Container Management' : 'Container ID not available',
    portainer: true
  });
  ...
  return card;
        ...
        async function fetchUpdates() {
            if (!isAdmin) return;
            // Suppress updates if system just started (< 10 mins) to prevent false positives or banner fatigue on fresh boot
            if (systemUptime < 600) {
        ...
        async function loadAllSettings() {
            try {
                ...
                const filter = data.dashboard_filter || localStorage.getItem('dashboard_filter') || 'all';
