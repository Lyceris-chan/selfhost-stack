            ...
            try {
                ...
                const entries = Object.entries(catalog)
                    .filter(([id]) => {
                        ...
                        // Also hide services that are clearly database/helper but not in our list
        ...
        function createServiceCard(id, meta, hardened = false) {
            ...
            // Remove nav arrow for services without a functional page (like Unbound)
            ...
            const pChip = createChipElement(id, { label: 'Portainer', icon: 'settings', variant: 'admin tonal', tooltip: 'Direct Container Management', portainer: true });
        ...
        /**
         * Dynamically adjusts the font size and padding of chips in service cards.
         * Why: The number of chips and their labels vary per service. We want to maximize 
         * information density without breaking the card layout or overflowing on smaller screens.
         * This ensures a consistent visual rhythm even with long service names or many tags.
         */
        ...
        function getServiceUrl(serviceId, defaultUrl) {
            ...
            if (hasDesec && (currentHostname.endsWith("$DESEC_DOMAIN") || isHttps)) {
                const port = window.location.port ? ":" + window.location.port : "";
        ...
        const DEFAULT_ODIDO_API_KEY = "$HUB_API_KEY";
        ...
        function generateM3Palette(seedHex) {
            ...
            if (isDark) {
                tokens.surfaceDim = hexFromArgb(palette.get(6));
                tokens.surfaceBright = hexFromArgb(palette.get(24));
                tokens.surfaceContainerLowest = hexFromArgb(palette.get(4));
                tokens.surfaceContainerLow = hexFromArgb(palette.get(10));
                tokens.surfaceContainer = hexFromArgb(palette.get(12));
                tokens.surfaceContainerHigh = hexFromArgb(palette.get(17));
                tokens.surfaceContainerHighest = hexFromArgb(palette.get(22));
            } else {
                tokens.surfaceDim = hexFromArgb(palette.get(87));
                tokens.surfaceBright = hexFromArgb(palette.get(98));
                tokens.surfaceContainerLowest = hexFromArgb(palette.get(100));
                tokens.surfaceContainerLow = hexFromArgb(palette.get(96));
                tokens.surfaceContainer = hexFromArgb(palette.get(94));
                tokens.surfaceContainerHigh = hexFromArgb(palette.get(92));
                tokens.surfaceContainerHighest = hexFromArgb(palette.get(90));
            }
