...
function createServiceCard(id, meta, hardened = false) {
  ...
  return card;
        ...
        function generateOdidoSignInUrl() {
            ...
            // Generate OAuth URL matching Odido.Authenticator implementation
            // The tool uses localhost:5000 as redirect_uri, which Odido's system
            // recognizes and redirects to /loginappresult with the encrypted token
            const clientId = 'OdidoMobileApp';
            const redirectUri = 'http://localhost:5000/token';
            const state = Math.random().toString(36).substring(2, 15);
            const nonce = Math.random().toString(36).substring(2, 15);
            
            const authUrl = `https://www.odido.nl/login?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&state=${state}&nonce=${nonce}&scope=openid%20profile%20email`;
            
            urlElement.href = authUrl;
            urlElement.textContent = authUrl;
