      ...
      .map(([id, meta]) => {
        ...
        if (id === 'immich' || id === 'immich-server') {
            normalized.chips.push({
                ...
                variant: 'tertiary', 
                icon: 'shield',
                tooltip: 'Your photos and videos are stored locally. Internet access is only used for maps and ML downloads.'
            });
        }
...
function createServiceCard(id, meta, hardened = false) {
  ...
  const containerInfo = activeContainers[portainerLookupId];
