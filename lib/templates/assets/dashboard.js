...
async function renderDynamicGrid() {
  try {
    ...
    const syncGrid = (gridId, items) => {
      ...
      // Dynamic grid adjustments based on count
      if (gridId === 'grid-apps') {
          const count = sorted.length;
          // General rule: Use 4 columns if divisible by 4 (and not too small) to avoid gaps
          // Otherwise default to 3 columns (where 3n+1 rule handles the last item)
          if (count > 4 && count % 4 === 0) {
              grid.classList.add('grid-4-cols');
          } else {
              grid.classList.remove('grid-4-cols');
          }
      }
    };
