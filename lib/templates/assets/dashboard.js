...
function createServiceCard(id, meta, hardened = false) {
  ...
  return card;
        ...
        function generateOdidoSignInUrl() {
            ...
            // Generate OAuth URL with proper parameters for Odido authentication
            const clientId = 'OdidoMobileApp';
            const redirectUri = 'https://www.odido.nl/loginappresult';
            const state = Math.random().toString(36).substring(2, 15);
            const nonce = Math.random().toString(36).substring(2, 15);
            
            const authUrl = `https://www.odido.nl/login?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=token&state=${state}&nonce=${nonce}&scope=openid%20profile%20email`;
