        ...
        async function switchSlot() {
            if (!await showDialog({
                title: 'Switch System Slot?',
                message: "This will trigger a full stack migration to the standby slot (A/B system). The dashboard will be unreachable for ~2 minutes.",
                confirmText: 'Switch & Migrate',
                confirmClass: 'btn btn-filled'
            })) return;

            showSnackbar("Slot swap initiated. Migrating system...");
            try {
                const res = await apiCall("/switch-slot", { method: 'POST' });
                if (res.ok) {
                    // Show same overlay as restart
                    document.body.innerHTML = `
                        <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100vh; background:var(--md-sys-color-surface); color:var(--md-sys-color-on-surface); font-family:sans-serif; text-align:center; padding:24px;">
                            <span class="material-symbols-rounded" style="font-size:64px; color:var(--md-sys-color-primary); margin-bottom:24px;">swap_horiz</span>
                            <h1>Migrating System Slots...</h1>
                            <p style="margin-top:16px; opacity:0.8;">The stack is being rebuilt on the standby slot. This process typically takes 1-2 minutes.</p>
            ...
            } catch(e) { showSnackbar("Slot switch failed: " + e.message); }
        }
