  ...
  return card;
        ...
        function filterCategory(cat) {
            ...
            if (cat === 'all') {
                ...
                // Logic: If any specific category is toggled OFF, 'All' must be OFF.
                // If ALL specific categories are ON, 'All' should be ON.
        ...
        function resetTheme() {
            if (confirm("Reset theme to default?")) {
                ...
                // Reload to re-apply defaults cleanly
        ...
        async function fetchUpdates() {
            ...
            // Fetch system uptime first if not set
            if (systemUptime === 0) {
                try {
        ...
        async function filterLogs() {
            ...
            try {
                ...
                el.innerHTML = ''; // Clear "Connecting..." message
                ...
                if (data.logs && data.logs.length > 0) {
                    ...
                    empty.className = 'body-small';
                    empty.style.textAlign = 'center';
                    empty.style.opacity = '0.7';
                    empty.style.padding = '24px';
                    empty.textContent = 'No logs found matching criteria.';
                ...
                showSnackbar("Failed to filter logs"); 
                const statusEl = document.getElementById('log-status');
        ...
        function updateAdminUI() {
            ...
            if (warningEl && isAdmin) {
                ...
                // ... rest of warning logic assumed handled or simplified
            } else if (warningEl) {
        ...
        async function fetchStatus() {
            ...
            try {
                ...
                if (hintWgeClients) {
                    ...
                    // Use smartphone icon for clients/devices
                    hintWgeClients.innerHTML = `<span class="material-symbols-rounded">smartphone</span> ${connected} Active / ${total} Total`;
        ...
        async function restoreSystem(filename) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    showSnackbar("Restore request failed.");
                }
            } catch (e) {
                showSnackbar("Error triggering restore: " + e.message);
            }
        }
        ...
        async function checkUpdates() {
            ...
            try {
                const res = await apiCall("/check-updates");
                if (res.ok) showSnackbar("Update check initiated.");
                else showSnackbar("Failed to start update check.");
            } catch (e) {
        ...
        async function backupSystem() {
            if (!await showDialog({
                title: 'Backup System?',
                message: "Create a full backup of all configurations and secrets?",
                confirmText: 'Backup'
            })) return;

            showSnackbar("Starting backup...");
            try {
                const res = await apiCall("/backup", { method: 'POST' });
                if (res.ok) showSnackbar("Backup started in background.");
                else showSnackbar("Backup request failed.");
            } catch (e) {
                showSnackbar("Error: " + e.message);
            }
        }
        ...
        async function uninstallStack() {
            if (!await showDialog({
                title: 'Uninstall System?',
                message: "DANGER: This will permanently DELETE all containers, volumes, and data. This action cannot be undone.\n\nAre you absolutely sure?",
                confirmText: 'Uninstall',
                confirmClass: 'btn btn-filled error' // Custom class for red button
            })) return;

            try {
                const res = await apiCall("/uninstall", { method: 'POST' });
                if (res.ok) {
                    document.body.innerHTML = '<div style="display:flex;justify-content:center;align-items:center;height:100vh;flex-direction:column;gap:16px;"><h1>System Uninstalled</h1><p>The privacy hub stack has been removed.</p></div>';
                }
            } catch (e) {
        ...
        function initLinkMode() {
            const useDomain = localStorage.getItem('link_mode_domain') === 'true';
            const sw = document.getElementById('link-mode-switch');
            const label = document.getElementById('link-mode-label');
            if (sw) sw.classList.toggle('active', useDomain);
            if (label) label.textContent = useDomain ? 'Use domain links' : 'Use IP links';
        }
        ...
        function toggleLinkMode() {
            ...
            localStorage.setItem('link_mode_domain', isActive);
            if (label) label.textContent = isActive ? 'Use domain links' : 'Use IP links';
            // Refresh grid to update links
            renderDynamicGrid();
        }

        function initTheme() {
            const seed = localStorage.getItem('theme_seed');
            const mode = localStorage.getItem('theme_mode');
            
            if (seed) applySeedColor(seed);
        ...
        function saveThemeSettings() {
            const seed = document.getElementById('theme-seed-hex').textContent;
            if (seed && seed.startsWith('#')) {
                localStorage.setItem('theme_seed', seed);
            }
            showSnackbar("Theme settings saved locally.");
        }

        function generateM3Palette(hex) {
            // Basic fallback palette generation if MCU fails or for static presets
            // In a real implementation, we'd use material-color-utilities more extensively
            // For now, let's just return the hex as primary and derive others simplistically
            return {
                primary: hex,
                onPrimary: '#FFFFFF',
                primaryContainer: hex + '40', // 25% opacity
                secondary: hex,
                surfaceContainer: hex + '10', // 6% opacity
                tertiary: hex 
            };
        }
        ...
        function applyThemeColors(colors) {
            ...
            // Apply only dynamic overrides, keep structure from CSS
            if (window.MaterialColorUtilities) {
                 // Use MCU if available (loaded in HTML)
                 const { themeFromSourceColor, applyTheme } = window.MaterialColorUtilities;
                 const theme = themeFromSourceColor(argbFromHex(colors.primary), [
                    {name: "custom-1", value: argbFromHex(colors.primary), blend: true}
                 ]);
                 
                 const isDark = !document.documentElement.classList.contains('light-mode');
                 const scheme = isDark ? theme.schemes.dark : theme.schemes.light;
                 
                 for (const [key, value] of Object.entries(scheme.props)) {
                     // Convert camelCase to kebab-case --md-sys-color-key
                     const token = key.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
                     r.style.setProperty(`--md-sys-color-${token}`, hexFromArgb(value));
                 }
            } else {
                // Fallback
                r.style.setProperty('--md-sys-color-primary', colors.primary);
                r.style.setProperty('--md-sys-color-primary-container', colors.primaryContainer);
            }
        }
        ...
        window.scanProfiles = scanProfiles;
        window.resetTheme = resetTheme;
        window.applySeedColor = applySeedColor;
        window.purgeUnusedImages = purgeUnusedImages;
        window.closeProjectSizeModal = closeProjectSizeModal;
