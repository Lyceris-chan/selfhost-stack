...
function createChipElement(id, chip) {
  ...
  if (isObject && chip.portainer) {
    ...
    chipEl.onclick = (e) => {
      ...
      let lookupId = id;
      if (id === 'immich') lookupId = 'immich-server';
      if (id === 'cobalt') lookupId = 'cobalt-web';
...
async function renderDynamicGrid() {
  try {
    ...
    const entries = Object.entries(catalog)
      .filter(([id]) => {
        let containerId = id;
        if (id === 'immich') containerId = 'immich-server';
        if (id === 'cobalt') containerId = 'cobalt-web';
      ...
      .map(([id, meta]) => {
        ...
        if (id === 'immich' || id === 'immich-server') {
            normalized.chips.push({
                ...
                variant: 'tertiary', 
                icon: 'cloud_sync',
                tooltip: 'Internet connectivity required: Maps display, machine learning models, and reverse geocoding. All photos/videos stored locally.'
            });
        }
...
function createServiceCard(id, meta, hardened = false) {
  ...
  let portainerLookupId = id;
  if (id === 'immich') portainerLookupId = 'immich-server';
  if (id === 'cobalt') portainerLookupId = 'cobalt-web';
  ...
  if (meta.category === 'apps') {
      // Check chips for vpn/lan logic if available, or default to generic
      if (meta.chips && meta.chips.some(c => c.icon === 'vpn_lock')) {
          icons.push({icon: 'vpn_lock', color: 'var(--md-sys-color-primary)', title: 'VPN Protected'});
      } else if (meta.chips && meta.chips.some(c => c.icon === 'lan')) {
          icons.push({icon: 'lan', color: 'var(--md-sys-color-secondary)', title: 'Direct Access'});
      }
  }
  ...
  return card;
        ...
        function filterCategory(cat) {
            ...
            if (cat === 'all') {
                ...
                if (!isActive) {
                    // FIXED: Turn everything ON - activate all category chips
                    chip.classList.add('active');
                    mainChips.forEach(c => c.classList.add('active'));
                } else {
                    // User is turning OFF "All" - deactivate all
                    chip.classList.remove('active');
                    mainChips.forEach(c => c.classList.remove('active'));
                    // Ensure at least one stays active (fallback to apps)
                    const appsChip = document.querySelector('.filter-chip[data-target="apps"]');
                    if (appsChip) appsChip.classList.add('active');
                }
            } else {
            ...
            try {
                ...
                if (pendingUpdates.length > 0) {
                    ...
                    if (dismissed === pendingUpdates.join(',')) {
                        ...
                        // Check if dismissed within last 24h? For now, exact match logic is fine.
                        // Ensure banner is only shown if not dismissed
                        if (banner) {
                            banner.classList.remove('hidden-banner');
        ...
        async function showServiceModal(name) {
            ...
            let lookupId = name;
            if (name === 'immich') lookupId = 'immich-server';
            if (name === 'cobalt') lookupId = 'cobalt-web';
        ...
        async function fetchContainerIds() {
            try {
                ...
                document.querySelectorAll('.portainer-chip').forEach(pChip => {
                    ...
                    let portainerLookupId = serviceId;
                    if (serviceId === 'immich') portainerLookupId = 'immich-server';
                    if (serviceId === 'cobalt') portainerLookupId = 'cobalt-web';
        ...
        async function saveDesecConfig() {
            ...
            // FIXED: DeSEC endpoint doesn't exist - show informative message
            showSnackbar("DeSEC configuration must be set via environment variables during deployment. Dashboard-based config coming soon.", "Dismiss");
            // TODO: Implement /api/config-desec endpoint in backend to support dynamic DeSEC configuration
        }
        ...
        async function fetchStatus() {
            ...
            try {
                ...
                if (hintVpnStatus) {
                    ...
                    const isUp = g.status === "up" || containerRunning === 'healthy' || containerRunning === 'up';
                    ...
                    hintVpnStatus.style.color = (isUp && isHealthy) ? 'var(--md-sys-color-success)' :
                                               (isUp ? 'var(--md-sys-color-warning)' :
        ...
        async function scanProfiles() {
            const btn = document.getElementById('scan-profiles-btn');
        ...
        function renderClientList(clients) {
            ...
            if (!clients || clients.length === 0) {
                list.innerHTML = '<div style="padding: 24px; text-align: center; opacity: 0.6;"><span class="material-symbols-rounded" style="font-size: 48px; line-height: 1; display: block; margin-bottom: 8px;">smartphone</span><p class="body-medium">No clients configured</p></div>';
                return;
            }
