  ...
  try {
      ...
      .map(([id, meta]) => {
        ...
        if (id === 'immich' || id === 'immich-server') {
            normalized.chips.push({
                label: '', 
                icon: 'public', 
                tooltip: 'Internet: Maps, Geocoding & ML Models'
            });
        }
    ...
    const syncGrid = (gridId, items) => {
      ...
      if (count > 0) {
          ...
          // Use 4 columns when:
          // - Count >= 4 (minimum to make sense)
          // - AND 4-col has equal or better score, OR count is large enough to benefit from density
          const use4Cols = (count >= 4) && (
              score4 < score3 ||                           // 4-col is clearly better
              (score4 === score3 && remainder4 === 0) ||   // Tie-breaker: prefer perfect 4-col
              (count >= 8 && score4 <= score3)             // Large grids: prefer 4-col density
          );
...
function createServiceCard(id, meta, hardened = false) {
  ...
  return card;
        ...
        function startLogStream() {
            ...
            logSource.onopen = () => {
                if(status) { status.textContent = 'Live'; status.style.color = 'var(--md-sys-color-success)'; }
            };
            ...
            logSource.onerror = () => {
                if(status) { status.textContent = 'Connecting...'; status.style.color = 'var(--md-sys-color-warning)'; }
            };
        }
