...
function createServiceCard(id, meta, hardened = false) {
  ...
  if (meta.local || (meta.chips && meta.chips.some(c => c.icon === 'shield'))) {
      ...
      shield.className = 'material-symbols-rounded';
      shield.textContent = 'shield';
      shield.style.cssText = 'font-size: 20px; color: var(--md-sys-color-tertiary); margin-left: 8px; cursor: help;';
      shield.title = 'Local Only: No internet access';
        ...
        function filterCategory(cat) {
            ...
            if (cat === 'all') {
                ...
                if (!isActive) {
                    ...
                    // mainChips.forEach(c => c.classList.add('active')); // Removed to prevent jarring effect
                    // Instead, deselect others to show "All" mode
                    mainChips.forEach(c => c.classList.remove('active'));
                } else {
                    // Already active, ensure others are off? Or do nothing.
                    // If we want "All" to mean "Only All Chip Active", we ensure others are off.
                    mainChips.forEach(c => c.classList.remove('active'));
                }
            } else {
