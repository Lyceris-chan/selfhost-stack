 ...
 * 
 * This module implements the interactive functionality for the Privacy Hub
 * dashboard, including service management, admin authentication, VPN status
 * monitoring, and dynamic theming. All code adheres to Material Design 3
 * principles and the Google JavaScript Style Guide.
 * 
 * Architecture:
 *   - Event-driven with async/await for API calls
 *   - Modular functions for maintainability
 *   - State management via sessionStorage and localStorage
 *   - Material Design 3 UI components and interactions
 * 
 * Key Features:
 *   - Dynamic service card rendering from API catalog
 *   - Admin mode with session management
 *   - Real-time VPN and system metrics
 *   - WireGuard profile management
 *   - SSL certificate status monitoring
 *   - Material Design 3 dynamic color theming
 *   - Privacy mode for sensitive information redaction
 * 
 * API Integration:
 *   All API calls use the apiCall() wrapper which handles:
 ...
 *   - Material Color Utilities (MCU) for theming
 * 
 * References:
 *   - Material Design 3: https://m3.material.io/
 *   - Google JavaScript Style Guide
 *   - Privacy Hub API Documentation
 * 
 ...
 * Uses sessionStorage for temporary data (admin state)
 * and localStorage for persistent preferences (theme, privacy mode).
 * ============================================ */

/** 
 * Service catalog loaded from API.
 * Maps service IDs to their metadata (name, description, category, etc.)
 * @type {Object<string, Object>}
 */
let serviceCatalog = {};

function resolveContainerId(id) {
    if (id === 'immich') return 'immich-server';
    if (id === 'cobalt') return 'cobalt-web';
    return id;
}
...
function createChipElement(id, chip) {
  ...
  if (isObject && chip.portainer) {
    ...
    chipEl.onclick = (e) => {
      e.preventDefault();
      e.stopPropagation();
      // Handle special container ID mappings
      const lookupId = resolveContainerId(id);
      const info = containerIds[lookupId];
  ...
  if (isObject && chip.icon) {
    ...
    if (label) chipEl.appendChild(document.createTextNode('\u00A0')); // Non-breaking space
  }
  ...
  try {
    ...
    const entries = Object.entries(catalog)
      .filter(([id]) => {
        ...
        const info = activeContainers[containerId];
        ...
        const adminOnlyServices = ['watchtower', 'odido-booster', 'adguard', 'unbound', 'wg-easy', 'gluetun', 'portainer'];
        if (!isAdmin && adminOnlyServices.includes(id)) return false;
      ...
      .map(([id, meta]) => {
        ...
        if (id === 'vert') {
            normalized.description = "Local file converter for images, audio, documents, and video files. GPU-accelerated local processing.";
            normalized.chips.push({
                label: '', 
                variant: 'tertiary', 
                icon: 'shield',
                tooltip: 'This service cannot connect to the internet; all file processing stays local on your hardware.'
            });
        }
        if (id === 'immich' || id === 'immich-server') {
            normalized.chips.push({
                label: '', 
                variant: 'tertiary', 
                icon: 'cloud_sync',
                tooltip: 'Internet access is required for map tiles and machine learning model downloads. Personal media remains local.'
            });
        }
    ...
    const syncGrid = (gridId, items) => {
      ...
      // Dynamic grid adjustments based on item count
      // Strategy: Choose between 3 and 4 columns based on which creates a cleaner layout
      // CSS handles orphan spanning automatically for both grid types
      //
      // Decision logic (prefer layout with fewer/no orphans):
      // - Perfect fit (remainder=0) is always preferred
      // - 4-col handles: 0, 2, 3 orphans well (2 span half each, 3 span 2+1+1)
      // - 3-col handles: 0, 1, 2 orphans well (1 spans full, 2 stay normal)
      // - For large counts, prefer 4-col when it reduces visual gaps
      const count = sorted.length;
      if (count > 0) {
          const remainder3 = count % 3;
          const remainder4 = count % 4;

          // Calculate "badness" score - lower is better
          // 0 orphans = 0 (perfect), 1-2 orphans = 1 (CSS handles), 3 orphans in 3-col = 2 (impossible)
          const score3 = remainder3 === 0 ? 0 : 1;
          const score4 = remainder4 === 0 ? 0 : (remainder4 === 1 ? 1.5 : 1); // 1 orphan slightly worse than 2-3

          // Use 4 columns ONLY if it's cleanly divisible by 4 and NOT by 3, or if it's much better.
          // This enforces the "3x3x3..." rule preference for 9, 10, 11 items.
          const use4Cols = (count >= 4) && (
              score4 < score3 ||                           // 4-col is clearly better (e.g. 8 items: 0 vs 1)
              (remainder4 === 0 && remainder3 !== 0)       // Divisible by 4 but not 3 (e.g. 4, 8, 16)
          );
    ...
    // Hide "Service utilities" label if grid-tools is empty in non-admin mode
    const toolsGrid = document.getElementById('grid-tools');
    const utilsLabel = document.getElementById('service-utilities-label');
    if (toolsGrid && utilsLabel && !isAdmin && toolsGrid.children.length === 0) {
      utilsLabel.style.display = 'none';
    } else if (utilsLabel) {
      utilsLabel.style.display = '';
    }
...
function createServiceCard(id, meta, hardened = false) {
  ...
  // Wrapper for Status/Arrow transition
  const statusWrapper = document.createElement('div');
  statusWrapper.className = 'status-arrow-wrapper';
  ...
  text.className = 'label-medium status-text'; // Added status-text class for easier selection
  ...
  if (meta.url && meta.url !== '#' && meta.url !== '') {
    ...
    statusWrapper.appendChild(navArrow);
  }

  actionsWrap.appendChild(statusWrapper);
  ...
  // Map container IDs for Portainer lookup
  const portainerLookupId = resolveContainerId(id);
  
  // FIXED: Check multiple possible container name variations
  const containerInfo = containerIds[portainerLookupId] || 
                        containerIds[id] || 
                        containerIds['hub-' + id] ||
                        containerIds['hub-' + portainerLookupId];
  const hasContainerId = containerInfo && containerInfo.id;
  
  const pChip = createChipElement(id, {
    label: 'Portainer',
    icon: 'settings',
    variant: 'admin tonal',
    tooltip: hasContainerId ? 'Direct Container Management' : 'Loading container ID...',
    portainer: true
  });
  
  // Store the service ID for later updates
  pChip.dataset.serviceId = id;
  pChip.classList.add('portainer-chip');
  
  // Only gray out if container ID is not available
  if (!hasContainerId) {
    pChip.style.opacity = '0.5';
    pChip.style.cursor = 'not-allowed';
    pChip.onclick = (e) => {
      e.preventDefault();
      e.stopPropagation();
    };
  ...
  // Replaced chip with header icon
  const icons = [];

  if (meta.local || (meta.chips && meta.chips.some(c => c.icon === 'shield'))) {
      icons.push({icon: 'shield', color: 'var(--md-sys-color-tertiary)', title: 'Local Only: No internet access'});
  }
  ...
  icons.forEach(ic => {
      ...
      span.style.cssText = `font-size: 20px; color: ${ic.color}; margin-left: 8px; cursor: help;`;
      span.title = ic.title;
  ...
  if (Array.isArray(meta.chips)) {
    meta.chips.filter(c => c.icon !== 'shield' && c.icon !== 'vpn_lock' && c.icon !== 'lan').forEach((chip) => {
  ...
  return card;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Shift+X trigger for update UI testing
                ...
                if (e.shiftKey && e.key.toLowerCase() === 'x') {
                    if (isAdmin) {
                        ...
                        if (banner) {
                            ...
                            banner.style.display = banner.classList.contains('hidden-banner') ? 'none' : 'block';
                            // Force show list even if empty for testing
                            const list = document.getElementById('update-list');
                            if (list) list.textContent = "Manual update trigger (Testing Mode)";
                            pendingUpdates = ['manual-test-service'];
                        }
                    }
                }
            });
        ...
        // Dynamic API URL Construction to handle both LAN and localhost scenarios
        const getApiBaseUrl = () => {
            const host = window.location.hostname;
            const protocol = window.location.protocol;
            // Use port 55555 for API, matching the host of the dashboard
            return `${protocol}//${host}:55555/api`;
        };

        const API = getApiBaseUrl();
        const ODIDO_API = getApiBaseUrl() + "/odido-proxy";
        ...
        function filterCategory(cat) {
            ...
            if (cat === 'all') {
                ...
                if (!isActive) {
                    ...
                    // Turn everything OFF (except maybe one default?) - Request says "untoggles one of the categories" implies specific behavior.
                    // "toggling all services should toggle all category cards on so users can disable them afterwards while all services stays on until a user untoggles one of the categories"
                    // If 'all' is active and clicked -> disable all? Or just disable 'all' chip? 
                    // Let's implement: Click All -> Activates All. Click All again -> Deactivates All.
                ...
                // If any main category is unchecked, 'All' should be unchecked
                ...
                // If NO chips are active, maybe activate the clicked one to prevent empty state?
        ...
        let containerIds = {};
        let pendingUpdates = [];
        let systemUptime = 0; // Track uptime globally

        /**
         * Updates the UI to reflect the current admin authentication state.
         * 
         * This function is called immediately on page load and whenever the
         * admin state changes (login/logout). It controls visibility of
         * admin-only elements via the 'admin-mode' class on the body element.
         * 
         * Visual Changes:
         *   - Toggles body.admin-mode class for CSS-based visibility
         *   - Updates admin button icon (lock_person vs admin_panel_settings)
         *   - Changes button background color when authenticated
         *   - Updates tooltip text
         *   - Shows/hides session cleanup warning
         * 
         * Security Consideration:
         *   CSS-based hiding is supplemented by server-side access control.
         *   Admin-only API endpoints require valid session token.
         * 
         * @function updateAdminUI
         * @returns {void}
         */
        function updateAdminUI() {
            // Use immediate display toggle to prevent flash of admin content
            ...
            // Force immediate reflow to ensure CSS takes effect before further DOM operations
            ...
            const rollbackSwitch = document.getElementById('rollback-backup-switch');
            if (rollbackSwitch) {
                const rollbackEnabled = localStorage.getItem('rollback_backup') === 'true';
                rollbackSwitch.classList.toggle('active', rollbackEnabled);
            }

            // Ensure System & Logs is visible and active when admin logs in
            const logsChip = document.querySelector('.filter-chip[data-target="logs"]');
            if (isAdmin && logsChip && !logsChip.classList.contains('active')) {
                // Auto-activate logs section for admin users
                logsChip.classList.add('active');
                const logsSection = document.querySelector('section[data-category="logs"]');
                if (logsSection) {
            ...
            if (warningEl && isAdmin) {
                ...
                if (sessionCleanupEnabled) {
                    ...
                    if (wText) wText.textContent = 'Session auto-cleanup is disabled. Admin access is persistent on this device/browser until you manually logout. This session token is stored locally.';
                }
            } else if (warningEl) {
        ...
        async function toggleRollbackBackup() {
            const toggle = document.getElementById('rollback-backup-switch');
            const isActive = toggle.classList.toggle('active');
            localStorage.setItem('rollback_backup', isActive ? 'true' : 'false');
            await syncSettings();
            showSnackbar(isActive ? "Rollback snapshots enabled" : "Rollback snapshots disabled");
        }

        // Initialize rollback backup to enabled by default for security
        document.addEventListener('DOMContentLoaded', () => {
            // Always default to enabled if not explicitly set
            if (localStorage.getItem('rollback_backup') === null) {
                localStorage.setItem('rollback_backup', 'true');
            }
            // Update the switch UI to reflect the stored state
            const rollbackSwitch = document.getElementById('rollback-backup-switch');
            if (rollbackSwitch) {
                const rollbackEnabled = localStorage.getItem('rollback_backup') !== 'false';
                rollbackSwitch.classList.toggle('active', rollbackEnabled);
            }

            // Auto-scan profiles on load if admin
            if (isAdmin) {
                setTimeout(scanProfiles, 1000);
            }
        });

        /**
         * Toggles admin mode on or off with confirmation.
         * 
         * Behavior:
         *   - If logged out: Opens the sign-in modal
         *   - If logged in: Shows confirmation dialog, then logs out
         * 
         * On successful logout:
         *   - Clears isAdmin flag and sessionStorage
         *   - Removes session token
         *   - Updates UI to hide admin controls
         *   - Syncs settings to server
         * 
         * Security:
         *   - Logout requires user confirmation
         *   - Session token is immediately invalidated
         *   - All admin state is cleared from browser
         * 
         * @async
        ...
        function dismissMacAdvisory() {
            ...
            if (banner) banner.classList.add('hidden-banner');
            localStorage.setItem('mac_advisory_dismissed_v3', 'true');
        }

        function initMacAdvisory() {
            const banner = document.getElementById('mac-advisory');
            if (banner) {
                if (localStorage.getItem('mac_advisory_dismissed_v3') === 'true') {
        ...
        async function fetchUpdates() {
            ...
            // FIXED: Fetch system uptime first if not set
            if (systemUptime === 0) {
                try {
                    const healthRes = await apiCall("/system-health");
                    if (healthRes.ok) {
                        const healthData = await healthRes.json();
                        systemUptime = healthData.uptime || 0;
                    }
                } catch (e) {
                    console.warn("Failed to fetch uptime:", e);
                }
            }
            
            // Suppress updates if system just started (< 10 mins)
            if (systemUptime < 600) {
                console.log("System startup period: Update check deferred.");
                return;
            }
            try {
        ...
        async function showServiceModal(name) {
            ...
            const lookupId = resolveContainerId(name);

            const containerInfo = containerIds[lookupId] || containerIds[name];
            ...
            try {
                const rbRes = await apiCall("/rollback-status?service=" + name);
                const rbData = await rbRes.json();
                if (rbData.available) {
                    actions.appendChild(createModalBtn('Rollback version', 'history', (e) => rollbackService(name, e), 'outlined', 'border-color:var(--md-sys-color-primary)'));
                }
            } catch(e) {}
            ...
            // Portainer Button
            actions.appendChild(createModalBtn('Manage in Portainer', 'settings_input_component', () => window.open(portainerLink, '_blank'), 'filled'));
        ...
        async function startBatchUpdate() {
            ...
            if (btn) {
                btn.disabled = true;
                btn.innerHTML = `<span class="material-symbols-rounded" style="animation: spin 2s linear infinite;">sync</span> Starting...`;
            }
        ...
        async function rollbackService(name, event) {
            let targetHash = null;

            try {
                const historyRes = await apiCall("/rollback-list?service=" + name);
                const historyData = await historyRes.json();

                if (historyData.history && historyData.history.length > 1) {
                    // Show version selector if multiple versions exist
                    const options = historyData.history.map(item => {
                        const date = new Date(item.timestamp).toLocaleString();
                        return {
                            label: `${item.hash.substring(0, 8)} (${date})`,
                            value: item.hash
                        };
                    });
                    ...
                    const selection = await showDialog({
                        ...
                        message: "Multiple rollback points detected. Please choose which version of " + name + " you wish to restore:",
                        type: 'prompt',
                        placeholder: 'Select a version...',
                        confirmText: 'Rollback to selected',
                        isVersionPicker: true, // Special flag handled below
                        versionOptions: options
                    });

                    if (!selection) return;
                    targetHash = selection;
                } else if (historyData.history && historyData.history.length === 1) {
                    if (!await showDialog({
                        title: 'Rollback Service?',
                        message: "Revert " + name + " to the previous version? This will restore the source code state from before the last update and rebuild the container.",
                        confirmText: 'Rollback',
                        confirmClass: 'btn btn-filled'
                    })) return;
                    targetHash = historyData.history[0].hash;
                } else {
                    showSnackbar("No rollback versions found.", "Error");
                    return;
                }
            } catch(e) {
                console.error("Failed to fetch rollback history:", e);
                // Fallback to default behavior if list fails
                if (!await showDialog({
                    title: 'Rollback Service?',
                    message: "Revert " + name + " to the previous version?",
                    confirmText: 'Rollback',
                    confirmClass: 'btn btn-filled'
                })) return;
            }
            ...
            if (btn) {
                btn.disabled = true;
                btn.innerHTML = `<span class="material-symbols-rounded" style="animation: spin 2s linear infinite;">history</span> Reverting...`;
            }
            showSnackbar(`Initiating rollback for ${name}...`, "Dismiss");

            try {
                const res = await apiCall("/rollback-service", {
                    method: 'POST',
                    body: JSON.stringify({
                        service: name,
                        hash: targetHash
                ...
                showSnackbar(`${name} rollback complete.`, "Success");
                closeServiceModal();
                return true;
            } catch(e) {
                showSnackbar(`Rollback failed: ${e.message}`, "Error");
                return false;
            } finally {
                if (btn) {
                    btn.disabled = false;
        ...
        async function fetchContainerIds() {
            try {
                ...
                // FIXED: Update all Portainer chips with container IDs
                document.querySelectorAll('.portainer-chip').forEach(pChip => {
                    const serviceId = pChip.dataset.serviceId;
                    if (!serviceId) return;
                    
                    // Check multiple possible container name variations
                    const portainerLookupId = resolveContainerId(serviceId);
                    
                    const containerInfo = containerIds[portainerLookupId] || 
                                        containerIds[serviceId] || 
                                        containerIds['hub-' + serviceId] ||
                                        containerIds['hub-' + portainerLookupId];
                    const cid = containerInfo ? containerInfo.id : null;
                    
                    if (cid) {
                        // Enable the chip
                        pChip.style.opacity = '1';
                        pChip.style.cursor = 'pointer';
                        pChip.setAttribute('data-tooltip', 'Direct Container Management');
                        pChip.onclick = (e) => {
                            e.preventDefault();
                            e.stopPropagation();
                        ...
                        // Keep disabled with updated message
                        pChip.setAttribute('data-tooltip', 'Container ID unavailable');
                    }
                });

                // Update all portainer links (legacy)
        ...
        function updateProfileListDisplay() {
            ...
            items.forEach((item) => {
                ...
                const row = item.closest('.list-item');
                const icon = row.querySelector('.material-symbols-rounded'); // This picks the first one (profile icon)

                if (realName === realProfileName) {
                    if (icon) {
                        icon.textContent = 'vpn_lock';
                        icon.style.color = 'var(--md-sys-color-primary)';
                    }
                    if (row) {
                        row.style.background = 'var(--md-sys-color-secondary-container)';
                        row.style.borderColor = 'var(--md-sys-color-primary)';
                    }
                } else {
                    if (icon) {
                        icon.textContent = 'vpn_key';
                        icon.style.color = 'var(--md-sys-color-on-surface-variant)'; // Default color
                    }
                    if (row) {
        ...
        async function saveDesecConfig() {
            ...
            try {
                ...
                if (data.success) {
                    showSnackbar(data.message || "DeSec configuration saved");
                    // Clear sensitive fields
                    document.getElementById('desec-token-input').value = '';
                } else {
                    showSnackbar(data.error || "Failed to save DeSec config");
                }
            } catch (e) {
                showSnackbar("Error saving DeSec configuration");
                console.error("DeSec config error:", e);
            }
        }

        let lastStatusTime = Date.now();
        let lastByteCounts = { vpn_rx: 0, vpn_tx: 0, wge_rx: 0, wge_tx: 0 };
        let firstStatusFetch = true;
        ...
        async function fetchStatus() {
            ...
            try {
                ...
                if (hintVpnSpeed) {
                    if (!firstStatusFetch && deltaSec > 0) {
                        ...
                        hintVpnSpeed.innerHTML = `<span class="material-symbols-rounded">speed</span> --`;
                    }
                }
                lastByteCounts.vpn_rx = vpn_rx;
                lastByteCounts.vpn_tx = vpn_tx;
                firstStatusFetch = false;

                if (hintVpnStatus) {
                    // Check if container is running before marking as down
                    const containerRunning = data.services && data.services['gluetun'];
                    const isUp = g.status === "up";
                    const isHealthy = g.healthy || containerRunning === 'healthy';
                    // FIXED: Don't show "Connecting..." if we have a valid UP status from the API
                    const statusText = (isUp && isHealthy) ? "Connected" :
                                      (isUp ? "Connected" :
                                      (containerRunning === 'starting' ? "Connecting..." : "Down"));
                    hintVpnStatus.innerHTML = `<span class="material-symbols-rounded">vpn_lock</span> VPN: ${statusText}`;
                    hintVpnStatus.style.color = (isUp) ? 'var(--md-sys-color-success)' :
                                               (containerRunning === 'starting' ? 'var(--md-sys-color-on-surface-variant)' : 'var(--md-sys-color-error)');
                }
                ...
                if (hintVpnUsage) {
                    ...
                    hintVpnUsage.setAttribute('data-tooltip', 'Session: Usage since last connect. Total: Accumulated usage over time.');
                }
                ...
                if (hintWgeClients) {
                    ...
                    hintWgeClients.innerHTML = `<span class="material-symbols-rounded">group</span> ${connected} Active / ${total} Total`;
                    ...
                    for (const [name, status] of Object.entries(data.services)) {
                        ...
                        cards.forEach(card => {
                            ...
                            if (dot && txt && indicator) {
                                // Don't show "Connecting..." indefinitely - default to Online if service exists
                                if (status === 'unhealthy' && data.health_details && data.health_details[name]) {
                                    ...
                                    indicator.title = 'Service is online and operational';
                                    ...
                                    dot.className = 'status-dot down';
                                    indicator.title = 'Service is unreachable or stopped';
                                } else {
                                    // Unknown status - assume online if container exists
                                    txt.textContent = 'Online';
                                    dot.className = 'status-dot up';
        ...
        async function generateOdidoSignInUrl() {
            const container = document.getElementById('odido-login-url-container');
            const urlElement = document.getElementById('odido-login-url');

            try {
                urlElement.textContent = "Generating...";
                container.style.display = 'block';

                const res = await apiCall("/odido-login-url");
                const data = await res.json();
                ...
                if (data.url) {
                    ...
                    urlElement.textContent = "Error: " + (data.error || "Failed to generate URL");
                }
            } catch (e) {
                urlElement.textContent = "Error: " + e.message;
            }
        }

        function copyOdidoSignInUrl() {
            const url = document.getElementById('odido-login-url').textContent;
            navigator.clipboard.writeText(url).then(() => {
                showToast('Sign-in URL copied to clipboard!');
            });
        }
        ...
        async function saveOdidoConfig() {
            ...
            const data = {};
            const callbackUrl = document.getElementById('odido-callback-url').value.trim();
            ...
            // If callback URL provided, extract and process the token
            if (callbackUrl) {
                if (st) {
                    st.textContent = 'Processing Odido authentication...';
                    st.style.color = 'var(--p)';
                }
                try {
                    // Parse the callback URL to extract the token parameter
                    const url = new URL(callbackUrl);
                    const urlToken = url.searchParams.get('token');

                    if (!urlToken) {
                        throw new Error('No token found in callback URL. Make sure you pasted the complete URL.');
                    }

                    if (st) {
                        st.textContent = 'Exchanging token with Odido API...';
                        st.style.color = 'var(--p)';
                    }

                    // Send the token to backend for processing (decrypt & exchange)
                    const tokenRes = await apiCall("/odido-exchange-token", {
                        method: 'POST',
                        body: JSON.stringify({ callback_token: urlToken })
                    });
                    const tokenResult = await tokenRes.json();

                    if (tokenResult.error) throw new Error(tokenResult.error);
                    if (!tokenResult.oauth_token) throw new Error('Failed to obtain OAuth token from Odido');

                    const oauthToken = tokenResult.oauth_token;
                    ...
                    // Now fetch User ID using the OAuth token
            ...
            try {
                ...
                document.getElementById('odido-callback-url').value = '';
        ...
        async function scanProfiles() {
            ...
            try {
                ...
                const res = await apiCall("/wireguard/profiles"); // Ensure this endpoint exists in wireguard router
                ...
                if (profiles.length === 0) {
                    list.innerHTML = `
                        <div style="display: flex; flex-direction: column; align-items: center; gap: 12px; opacity: 0.7;">
                            ...
                            <span class="body-medium">No profiles found</span>
                        </div>`;
                    return;
                }
                ...
                profiles.forEach(p => {
                    ...
                    text.className = 'list-item-text';
                    text.textContent = p;
                    text.dataset.realName = p; // For masking
                ...
                updateProfileDisplay(); // Apply masking/active state
                
            } catch (e) {
                list.innerHTML = `
                    <div style="display: flex; flex-direction: column; align-items: center; gap: 8px; color: var(--md-sys-color-error);">
                        <span class="material-symbols-rounded" style="font-size: 32px;">error</span>
                        <span class="body-medium">Scan failed</span>
                        <button onclick="scanProfiles()" class="btn btn-tonal btn-small">Retry</button>
                    </div>`;
            }
        }
        ...
        async function fetchProfiles() {
            ...
            try {
                ...
                if (!data.profiles || data.profiles.length === 0) {
                    el.innerHTML = '<div style="text-align:center; padding: 24px; opacity: 0.6;"><span class="material-symbols-rounded" style="font-size: 48px; line-height: 1; display: block; margin-bottom: 8px;">vpn_key</span><p class="body-medium">Click refresh to scan profiles</p></div>';
                    return;
                }

                // FIXED: Filter out 'active' symlink to prevent duplicate profile display
                const filteredProfiles = data.profiles.filter(p => p !== 'active' && p !== 'active.conf');
        ...
        async function uploadProfile() {
            ...
            const config = document.getElementById('prof-conf').value.trim();
            ...
            try {
                ...
                const actRes = await apiCall("/activate", {
                    method:'POST',
        ...
        function startLogStream() {
            ...
            // Use relative path for EventSource to go through Nginx proxy
            try {
                logSource = new EventSource(`/api/events?token=${sessionToken}`);

                logSource.onopen = () => {
                    logStreamRetryCount = 0;
                    if(status) { 
                        status.textContent = 'Live'; 
                        status.style.color = 'var(--md-sys-color-success)'; 
                    }
                    if (container) {
                        // Remove loading spinner on connection
                        const spinner = container.querySelector('.body-medium');
                        if (spinner && spinner.textContent.includes('Connecting')) {
                            container.innerHTML = '';
                        }
                    }
                };
                ...
                logSource.onmessage = (e) => {
                    ...
                    if (entry && container) {
                        // Remove spinner on first message if present
                        if (container.innerHTML.includes('spin 1s')) container.innerHTML = '';
                ...
                logSource.onerror = (error) => {
                    ...
                    if (logSource && logSource.readyState === EventSource.CLOSED) {
                        if (status) {
                            if (logStreamRetryCount >= MAX_LOG_RETRIES) {
                                status.textContent = 'Connection failed';
                                status.style.color = 'var(--md-sys-color-error)';
                            } else {
                                status.textContent = 'Reconnecting...';
                                status.style.color = 'var(--md-sys-color-warning)';
                            }
                        }
                        ...
                        if (logStreamRetryCount < MAX_LOG_RETRIES) {
                            setTimeout(() => {
                                if (isAdmin) startLogStream();
                            }, Math.min(1000 * Math.pow(2, logStreamRetryCount), 5000));
                        }
                    } else {
                        if (status) {
                ...
                if (status) {
                    status.textContent = 'Failed to start log stream';
                    status.style.color = 'var(--md-sys-color-error)';
                }
            }
        }
        ...
        function showDialog(options) {
            return new Promise((resolve) => {
                ...
                const existingPicker = inputContainer.querySelector('.version-picker');
                if (existingPicker) existingPicker.remove();

                if (options.isVersionPicker && options.versionOptions) {
                    inputContainer.style.display = 'block';
                    input.style.display = 'none';
                    confirmBtn.style.display = 'none'; // Hide confirm, selection will trigger resolve

                    const picker = document.createElement('div');
                    picker.className = 'version-picker';
                    picker.style.cssText = 'display: flex; flex-direction: column; gap: 8px; margin-top: 16px;';

                    options.versionOptions.forEach(opt => {
                        const btn = document.createElement('button');
                        btn.className = 'btn btn-outlined';
                        btn.style.cssText = 'justify-content: flex-start; text-align: left; padding: 12px;';
                        btn.innerHTML = `<span class="material-symbols-rounded" style="margin-right: 12px;">history</span> ${opt.label}`;
                        btn.onclick = () => {
                ...
                if (options.type === 'prompt' && !options.isVersionPicker) {
                    input.onkeydown = (e) => {
        ...
        function updateStrategyChange() {
            ...
            if (select.value === 'stable') {
                desc.textContent = "Stable: Use official release tags (Recommended).";
            } else {
        ...
        async function syncSettings() {
            ...
            const rollbackSwitch = document.getElementById('rollback-backup-switch');
            const rollbackBackup = rollbackSwitch ? rollbackSwitch.classList.contains('active') : false;
        ...
        async function resetTheme() {
            if (!await showDialog({
                title: 'Reset Theme?',
                message: "Reset all theme and category settings to default?",
                confirmText: 'Reset',
                confirmClass: 'btn btn-tonal'
            })) return;

            try {
                const defaultFilter = 'apps,system,dns,tools';
                // Reset server state
                await apiCall("/theme", {
                    method: 'POST',
                    body: JSON.stringify({
                        ...
                        theme: 'dark',
                        privacy_mode: false,
                        dashboard_filter: defaultFilter
                    })
                });
                // Clear local state but set default filter
                localStorage.removeItem('theme_seed');
                localStorage.removeItem('theme');
                localStorage.setItem('dashboard_filter', defaultFilter);
                localStorage.removeItem('privacy_mode');
                
                location.reload();
            } catch(e) {
                showSnackbar("Failed to reset theme: " + e.message);
            }
        }
        ...
        async function purgeUnusedImages(e) {
            ...
            const originalHtml = btn ? btn.innerHTML : 'Prune unused assets';
            if (btn) {
                btn.disabled = true;
                // Use a simpler loading state to avoid double icons
                btn.innerHTML = 'Pruning...';
            }
        ...
        async function loadAllSettings() {
            try {
                ...
                // 4. Dashboard Filter - FIXED: Properly restore filter state from server
                ...
                // Activate chips based on saved filter
                if (filter === 'all') {
                    // If 'all' was saved, activate all chips including 'all'
                    document.querySelectorAll('.filter-chip').forEach(c => c.classList.add('active'));
                } else {
                    const cats = filter.split(',').filter(c => c);
                    cats.forEach(cat => {
                        const chip = document.querySelector(`.filter-chip[data-target="${cat}"]`);
                        if (chip) chip.classList.add('active');
                    });
                    
                    // Check if all main categories are active, if so, activate 'all' chip too
                    const mainChips = Array.from(document.querySelectorAll('.filter-chip:not([data-target="all"]):not([data-target="logs"])'));
                    const allActive = mainChips.every(c => c.classList.contains('active'));
                    if (allActive) {
                        const allChip = document.querySelector('.filter-chip[data-target="all"]');
        ...
        /**
         * Toggles privacy mode (Safe Display Mode) on or off.
         * 
         * Privacy mode redacts sensitive information from the dashboard:
         *   - WireGuard profile names → Masked IDs
         *   - IP addresses → Partial redaction
         *   - DNS endpoints → Hidden
         *   - System identifiers → Generic labels
         * 
         * Use Case:
         *   Enable when screensharing, recording tutorials, or displaying
         *   the dashboard in public settings to protect private data.
         * 
         * Persistence:
         *   Preference saved to localStorage and synced to server.
         *   Automatically restored on page reload.
         * 
         * Implementation:
         *   CSS class 'privacy-mode' on body triggers .sensitive element hiding
         *   and replacement with placeholder content via JavaScript.
         * 
        ...
        async function fetchCertStatus() {
            try {
                ...
                const certStatus = (data.status || data.type || "").toLowerCase();
                const isTrusted = certStatus.includes("trusted");
                const isSelfSigned = certStatus.includes("self-signed");
                const hasDesec = "$DESEC_DOMAIN" !== "";

                // Only update if status actually changed to prevent flashing
                const newClass = isTrusted ? "chip vpn" : (isSelfSigned ? "chip admin" : (certStatus === "rate limited" ? "chip tertiary" : "chip tertiary"));
                const newContent = isTrusted ? '<span class="material-symbols-rounded" style="font-size:16px;">verified</span> Trusted' :
                                   (isSelfSigned ? '<span class="material-symbols-rounded" style="font-size:16px;">warning</span> Self-Signed' :
                                   (certStatus === "rate limited" ? '<span class="material-symbols-rounded" style="font-size:16px;">timer</span> Rate Limited' : (data.status || data.type || "Unknown")));
                
                if (badge.className !== newClass || badge.innerHTML !== newContent) {
                    badge.className = newClass;
                    badge.innerHTML = newContent;
                }

                if (isTrusted) {
                    badge.dataset.tooltip = "✓ Globally Trusted: Valid certificate from a recognized authority.";
                } else if (isSelfSigned) {
                    badge.dataset.tooltip = "⚠ Self-Signed (Local): Devices will show security warnings. deSEC configuration recommended.";
                } else if (certStatus === "rate limited") {
                    badge.dataset.tooltip = data.error || "Let's Encrypt rate limit reached. Re-attempting automatically.";
                } else if (!certStatus || certStatus === "checking...") {
                    badge.className = "chip tertiary";
                    badge.innerHTML = '<span class="material-symbols-rounded" style="font-size:16px; animation: spin 1s linear infinite;">sync</span> Checking...';
                    badge.dataset.tooltip = "Verifying certificate status...";
                } else {
        ...
        async function fetchSystemHealth() {
            try {
                ...
                systemUptime = uptime; // Update global state
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Initialize UI state immediately to prevent flickering
            updateAdminUI();
            initPrivacyMode();
            initLinkMode();
            initTheme();
            ...
            if (isAdmin) {
                setTimeout(() => {
                    // Don't auto-fetch profiles on load - user must click scan button
                    fetchWgClients();
                    fetchOdidoStatus();
                    startLogStream();
                    fetchUpdates();
        ...
        function renderClientList(clients) {
            ...
            // Add refresh button to header if not present
            const headerActions = list.parentElement.querySelector('.card-header-actions');
            if (headerActions && !headerActions.querySelector('.refresh-clients-btn')) {
                const refreshBtn = document.createElement('button');
                refreshBtn.className = 'btn btn-icon refresh-clients-btn';
                refreshBtn.innerHTML = '<span class="material-symbols-rounded">refresh</span>';
                refreshBtn.onclick = fetchWgClients;
                headerActions.insertBefore(refreshBtn, headerActions.firstChild);
            }

            if (!clients || clients.length === 0) {
                list.innerHTML = '<div style="padding: 24px; text-align: center; opacity: 0.6;"><span class="material-symbols-rounded" style="font-size: 48px; line-height: 1; display: block; margin-bottom: 8px;">phonelink_off</span><p class="body-medium">No clients configured</p></div>';
                return;
            }
                ...
                row.innerHTML = `
                    <div style="display:flex; align-items:center; justify-content:space-between; width:100%; gap:12px;">
                        <div style="display:flex; align-items:center; gap:12px; flex-grow:1;">
                            <span class="material-symbols-rounded" style="color: ${statusColor};" title="${client.enabled ? 'Enabled' : 'Disabled'}">${client.name ? (client.name.toLowerCase().includes('phone') ? 'smartphone' : client.name.toLowerCase().includes('laptop') || client.name.toLowerCase().includes('desktop') ? 'computer' : client.name.toLowerCase().includes('tablet') ? 'tablet' : 'devices') : 'devices'}</span>
        ...
        async function showClientQr(id, name) {
            ...
            // FIXED: Center profile name in QR modal
            ...
            try {
                ...
                if (confRes.ok) {
                    ...
                    // Generate QR - Check if QRCode library is loaded
                    if (typeof QRCode !== 'undefined') {
                        container.innerHTML = ''; // Clear container
                        if (QRCode.toCanvas) {
                             const canvas = document.createElement('canvas');
                             container.appendChild(canvas);
                             QRCode.toCanvas(canvas, configText, { width: 256, margin: 1 }, function (error) {
                            ...
                            try {
                                ...
                                console.error("QRCode legacy init failed", e);
                                container.innerHTML = '<p class="body-medium" style="color:var(--md-sys-color-error);">QR Library Error</p>';
                            }
                        }
                    } else {
                        container.innerHTML = '<p class="body-medium" style="color:var(--md-sys-color-error);">QR Code library not loaded. Please refresh the page.</p>';
        ...
        async function backupSystem() {
            if (!await showDialog({
                title: 'System Backup',
                message: "Create a full backup of all secrets, configurations, and environment variables? This may take a few moments.",
                confirmText: 'Create Backup',
                confirmClass: 'btn btn-filled'
            })) return;

            try {
                showSnackbar("Starting system backup...");
                const res = await apiCall("/backup", { method: 'POST' });
                if (res.ok) {
                    showSnackbar("Backup initiated in the background.");
                } else {
                    showSnackbar("Failed to start backup.");
                }
            } catch (e) {
                showSnackbar("Error starting backup: " + e.message);
            }
        }

        /**
         * Opens the restore modal and fetches the list of available backups.
         */
        ...
        /**
         * Fetches and renders the list of available system backups.
         */
        async function fetchBackups() {
            ...
            try {
                ...
                container.innerHTML = '';
                if (backups.length === 0) {
                    container.innerHTML = '<div style="padding: 24px; text-align: center; opacity: 0.7;">No backups found in data/AppData/privacy-hub/backups</div>';
                    return;
                }
                ...
                backups.forEach(b => {
                    const row = document.createElement('div');
                    row.className = 'list-item';
                    row.style.cssText = 'margin: 4px 0; background: var(--md-sys-color-surface-container-high); border-radius: 8px; padding: 12px; display: flex; align-items: center; justify-content: space-between;';

                    const info = document.createElement('div');
                    info.style.cssText = 'display: flex; flex-direction: column; gap: 2px;';

                    const name = document.createElement('span');
                    name.className = 'label-large monospace';
                    name.textContent = b.filename;

                    const meta = document.createElement('span');
                    meta.className = 'body-small';
                    meta.style.opacity = '0.7';
                    meta.textContent = `${b.timestamp} • ${b.size.toFixed(2)} MB`;
                    ...
                    btn.onclick = () => restoreSystem(b.filename);
                ...
                container.innerHTML = '<p class="error" style="padding: 16px;">Failed to load backups.</p>';
            }
        }
        ...
        async function restoreSystem(filename) {
            if (!await showDialog({
                ...
                message: `Are you SURE you want to restore from "${filename}"? This will OVERWRITE all current settings and RESTART the entire stack immediately.`,
                confirmText: 'Restore Now',
                confirmClass: 'btn btn-filled'
            })) return;

            try {
                showSnackbar("Initiating system restoration...");
                closeRestoreModal();
                const res = await apiCall(`/restore?filename=${encodeURIComponent(filename)}`, { method: 'POST' });
                if (res.ok) {
                    showSnackbar("Restoration started. Dashboard will lose connection during restart.");
                    setTimeout(() => window.location.reload(), 15000);
                } else {
                    showSnackbar("Restoration failed to start.");
                }
            } catch (e) {
                showSnackbar("Error during restoration: " + e.message);
            }
        }
