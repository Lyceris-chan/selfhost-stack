...
# --- 4a. IPTABLES/NETFILTER SAFETY ---
log_info "Verifying iptables/NAT support for port publishing..."
NETFILTER_MODULES=(br_netfilter ip_tables iptable_nat nf_nat nf_conntrack xt_MASQUERADE)
for mod in "${NETFILTER_MODULES[@]}"; do
    if ! lsmod | awk '{print $1}' | grep -q "^${mod}$"; then
        sudo modprobe "$mod" 2>/dev/null && log_info "  loaded $mod" || log_warn "  could not load $mod (may be built-in)"
    fi
done
sudo sysctl -w net.bridge.bridge-nf-call-iptables=1 net.bridge.bridge-nf-call-ip6tables=1 >/dev/null 2>&1 || true
sudo sysctl -w net.ipv4.ip_forward=1 >/dev/null 2>&1 || true

NETFILTER_TEST_CHAIN="zima_netfilter_test"
NETFILTER_OK=true
if sudo iptables -t nat -N "$NETFILTER_TEST_CHAIN" 2>/dev/null || sudo iptables -t nat -F "$NETFILTER_TEST_CHAIN" 2>/dev/null; then
    if ! sudo iptables -t nat -A "$NETFILTER_TEST_CHAIN" -s "$DOCKER_SUBNET" -j MASQUERADE >/dev/null 2>&1; then
        NETFILTER_OK=false
    fi
    sudo iptables -t nat -F "$NETFILTER_TEST_CHAIN" >/dev/null 2>&1 || true
    sudo iptables -t nat -X "$NETFILTER_TEST_CHAIN" >/dev/null 2>&1 || true
fi

if [ "$NETFILTER_OK" = false ]; then
    log_crit "Host iptables backend rejected NAT rules (check br_netfilter/xt_MASQUERADE support)."
    log_crit "Try switching to iptables-legacy or enabling bridge netfilter, then rerun the script."
    exit 1
fi
...
# Track which certificate is active for UI messaging
CERT_SOURCE="self_signed"
CERT_HOST="$LAN_IP"
CERT_BADGE_LABEL="Local self-signed"
CERT_BADGE_CLASS="warning"
CERT_IMPLICATION="Browsers will warn on HTTPS; trust this cert locally to use encrypted DNS (DoH/DoT)."
...
if [ "$CERT_SOURCE" = "letsencrypt" ]; then
    CERT_BADGE_LABEL="Let's Encrypt (deSEC)"
    CERT_BADGE_CLASS="success"
    CERT_IMPLICATION="Publicly trusted certificate for $CERT_HOST; encrypted DNS should work without warnings."
elif [ -n "$DESEC_DOMAIN" ]; then
    CERT_BADGE_LABEL="deSEC domain (self-signed)"
    CERT_BADGE_CLASS="warning"
    CERT_IMPLICATION="Using a locally generated certificate for $CERT_HOST. Browsers will warn until it is trusted on your devices."
fi

MOBILE_CERT_NOTE="⚠ Local self-signed certificate (expect warnings until trusted)"
MOBILE_CERT_COLOR="var(--md-sys-color-warning)"
if [ "$CERT_SOURCE" = "letsencrypt" ]; then
    MOBILE_CERT_NOTE="✓ Valid Let's Encrypt certificate (no warnings)"
    MOBILE_CERT_COLOR="var(--md-sys-color-success)"
elif [ -n "$DESEC_DOMAIN" ]; then
    MOBILE_CERT_NOTE="⚠ Using a local certificate for $CERT_HOST (trust it on devices to silence warnings)"
fi
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .chip.warning {
            background: color-mix(in srgb, var(--md-sys-color-warning) 18%, transparent);
        ...
        <div class="grid-2">
            <div class="card">
                ...
                <div class="cert-status">
                    <span class="chip cert-chip $CERT_BADGE_CLASS">$CERT_BADGE_LABEL</span>
                    <p class="cert-note">$CERT_IMPLICATION</p>
                </div>
...
if [ -n "$DESEC_DOMAIN" ]; then
    cat >> "$DASHBOARD_FILE" <<EOF
            ...
            <div class="card">
                ...
                <p class="body-small" style="color:$MOBILE_CERT_COLOR; margin-top:12px;">$MOBILE_CERT_NOTE</p>
            </div>
EOF
else
    cat >> "$DASHBOARD_FILE" <<EOF
