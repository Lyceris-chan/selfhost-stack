...
def get_proxy_opener():
    ...
    proxy_handler = urllib.request.ProxyHandler({'http': f'http://__CONTAINER_PREFIX__gluetun:8888', 'https': f'http://__CONTAINER_PREFIX__gluetun:8888'})
    ...
    def do_POST(self):
        ...
        if self.path == '/theme':
            ...
            try:
                ...
                # Sync Xray settings
                if 'enable_xray' in data:
                    file_secrets['ENABLE_XRAY'] = 'true' if data['enable_xray'] else 'false'
                if 'xray_domain' in data:
                    file_secrets['XRAY_DOMAIN'] = data['xray_domain']
